...
async function echoTranscription(token: string, platformId: string, transcript: string): Promise<void> {
  ...
  try {
    await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      ...
      body: JSON.stringify({ chat_id: chatId, text: `🎙️ "${transcript}"` }),
    });
  } catch (err) {
    log.warn('Voice echo send failed', { err });
  }
}

async function transcribeVoice(base64Data: string, mimeType: string): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      log.warn('Groq transcription failed', { status: response.status });
      return null;
    }
    const json = (await response.json()) as { text?: string };
    return json.text?.trim() || null;
  } catch (err) {
    log.warn('Voice transcription request failed', { err });
    return null;
  }
}
...
function createVoiceTranscriptionInterceptor(
  ...
  return async (platformId, threadId, message) => {
    if (message.kind === 'chat-sdk') {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const attachments = content?.attachments as Array<Record<string, any>> | undefined;
      const voiceAtt = attachments?.find(
        (a) => (a.type === 'voice' || a.type === 'audio') && typeof a.data === 'string',
      );
      if (voiceAtt && !content.text) {
        try {
          const transcript = await transcribeVoice(
            voiceAtt.data as string,
            (voiceAtt.mimeType as string) || 'audio/ogg',
          );
          if (transcript) {
            content.text = transcript;
            content.attachments = (attachments as Array<Record<string, unknown>>).filter(
              (a) => a.type !== 'voice' && a.type !== 'audio',
            );
            log.info('Voice message transcribed', { platformId, chars: transcript.length });
            if (readEnvFile(['VOICE_ECHO']).VOICE_ECHO === 'true') {
              void echoTranscription(token, platformId, transcript);
            }
          } else {
            delete voiceAtt.data;
          }
        } catch (err) {
    ...
    return hostOnInbound(platformId, threadId, message);
  };
}
  ...
  factory: () => {
    ...
    const wrapped: ChannelAdapter = {
      ...
      async setup(hostConfig: ChannelSetup) {
        const withTranscription = createVoiceTranscriptionInterceptor(hostConfig.onInbound, token);
        const intercepted: ChannelSetup = {
          ...hostConfig,
          onInbound: createPairingInterceptor(botUsernamePromise, withTranscription, token),
        };
